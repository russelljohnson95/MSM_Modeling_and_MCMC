function [y,muscle_force_out_norm,controls] = Arm16_SimManager_controls_CRBF_6musc_wForce(theta)
...
for k = 1:nMuscles
    ...
    %     osimState.setTime
        numVar = osimState.getNY();
        for i = 0:numVar-1
            osimState.updY().set(i,state(i+1))
        end

        osimModel.computeStateVariableDerivatives(osimState);
        muscle_force = ArrayDouble();

        muscle_force = osimModel.getForceSet.get(k-1).getRecordValues(osimState);
        muscle_force_out(j,k) = muscle_force.getitem(0);
...
Kinematics_norm          = interp1(Time_SM,Kinematics,time_int);


y = [time_int',Kinematics_norm]; 
